...
const Dashboard: React.FC = () => {
  ...
  const fetchResumes = async () => {
    ...
    try {
      // SEEDHA 8082 (Resume Service) KO CALL
      ...
      if (response.ok) {
        ...
        const formattedResumes = rawData.map((res: any) => {
          let parsedContent = { summary: '', skills: '', experience: '', education: '' };
          if (res.content) {
      ...
      <aside className="w-64 bg-gray-900 text-white flex flex-col shadow-xl print:hidden">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          <button 
            ...
            className={`w-full flex items-center px-4 py-3 rounded-lg transition-colors ${currentView === 'dashboard' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`w-full flex items-center px-4 py-3 rounded-lg transition-colors ${currentView === 'create' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
          >
        ...
        <div className="p-4 border-t border-gray-800">
          <button 
            ...
            className="w-full flex items-center justify-center px-4 py-2 text-sm font-medium text-red-400 hover:bg-gray-800 hover:text-red-300 rounded-lg transition-colors"
          >
      ...
      <main className="flex-1 flex flex-col h-screen print:h-auto print:overflow-visible">
        <header className="h-20 bg-white shadow-sm flex items-center justify-between px-10 flex-shrink-0 print:hidden">
          <div className="text-gray-500">
            Overview / <span className="text-gray-900 font-medium capitalize">
              {currentView === 'dashboard' ? 'My Resumes' : currentView.replace('-', ' ')}
            </span>
          </div>
        ...
        <div className="p-10 flex-1 overflow-y-auto print:p-0 print:overflow-visible">
          ...
          {(currentView === 'create' || currentView === 'edit') && (
            <ResumeBuilder 
              existingResume={currentView === 'edit' ? selectedResume : null} 
              onSuccessReturn={() => setCurrentView('dashboard')} 
            />
          )}
          ...
          {currentView === 'preview' && selectedResume && (
             <div className="max-w-4xl mx-auto bg-white p-10 rounded-2xl shadow-lg border border-gray-100 print:shadow-none print:border-none print:p-0 print:max-w-full">
              ...
              <div className="print:text-black">
                <h1 className="text-5xl font-extrabold text-gray-900 mb-2">{userName}</h1>
